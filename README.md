...
- 完整发现 `.grok`、`.agents`、Claude、Cursor、插件和额外目录中的 Skills，并保留同名冲突信息
- 创建用户级或项目级 `SKILL.md`，管理自定义路径、忽略项、启停状态与调用元数据
- 添加或更新 stdio、HTTP、SSE MCP 服务，配置环境变量、请求头、OAuth 与各级超时
- 查看 MCP 实时连接状态和工具列表，完成 OAuth/初始化，并按服务器或单个工具启停
- 浏览市场目录，或从 Git、GitHub 和本地目录安装插件
- 查看、启停、更新和卸载插件，并可选择保留插件数据
- 添加、更新和移除 GitHub、Git 或本地插件市场
- 查看插件组件详情，校验 manifest，并预览或创建发布标签
- 管理项目级与用户级配置
